...
function findInstaller() {
  const candidates = [
    "提灯_2.4.5_x64-setup.exe",
    "Lantern_2.4.5_x64-setup.exe",
    "lantern_2.4.5_x64-setup.exe",
    "提灯_2.4.5_x64_en-US.msi",
  ];
...
function saveBuild(label) {
  ...
  const nsisRenamed = join(bundleDir, `提灯_2.4.5_x64-${label}-setup${ext}`);
  renameSync(found, nsisRenamed);
  console.log(`  NSIS: ${nsisRenamed}`);

  // Copy to dist-installer
  const dest = join(outDir, `Lantern_2.4.5_${label}${ext}`);
