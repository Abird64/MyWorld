...
function findInstaller() {
  const candidates = [
    "提灯_1.2.3_x64-setup.exe",
    "Lantern_1.2.3_x64-setup.exe",
    "lantern_1.2.3_x64-setup.exe",
    "提灯_1.2.3_x64_en-US.msi",
  ];
...
function saveBuild(label) {
  ...
  const nsisRenamed = join(bundleDir, `提灯_1.2.3_x64-${label}-setup${ext}`);
  renameSync(found, nsisRenamed);
  console.log(`  NSIS: ${nsisRenamed}`);

  // Copy to dist-installer
  const dest = join(outDir, `Lantern_1.2.3_${label}${ext}`);
